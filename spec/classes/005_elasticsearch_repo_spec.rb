  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      context 'Use stage type for ordering' do
        let(:params) do
          default_params.merge(:repo_stage => 'setup')
        end
      ...
      context 'Package pinning' do
        let(:params) do
          default_params.merge(:package_pin => true)
        end
      ...
      context 'Override repo key ID' do
        let(:params) do
          default_params.merge(:repo_key_id => '46095ACC8548582C1A2699A9D27D666CD88E42B4')
        end

        case facts[:osfamily]
        when 'Debian'
          context 'has override apt key' do
            it { is_expected.to contain_apt__source('elasticsearch').with(:key => '46095ACC8548582C1A2699A9D27D666CD88E42B4') }
          end
        when 'Suse'
          context 'has override yum key' do
            it { is_expected.to contain_exec('elasticsearch_suse_import_gpg').with(:unless => "test $(rpm -qa gpg-pubkey | grep -i '46095ACC8548582C1A2699A9D27D666CD88E42B4' | wc -l) -eq 1 ") }
          end
        end
      end
      ...
      context 'Override repo source URL' do
        let(:params) do
          default_params.merge(:repo_key_source => 'https://packages.elasticsearch.org/GPG-KEY-elasticsearch')
        end

        case facts[:osfamily]
        when 'Debian'
          context 'has override apt key source' do
            it { is_expected.to contain_apt__source('elasticsearch').with(:key_source => 'https://packages.elasticsearch.org/GPG-KEY-elasticsearch') }
          end
        when 'RedHat'
